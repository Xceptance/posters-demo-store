...
| :--- | :--- | :--- | :--- | :--- |
| Antigravity (AI) | 2026-05-03 | `❌ FAILED` | Localhost / Chrome 144 | de Hornisse returned 0 results. JA currency is ￥ (test updated). UK added. |
...
| Field | en-US | en-GB | de-DE | sv-SE | ja-JP |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Search Term | `bear` | `bear` | `Bär` | `björn` | `ダリア` |
| Expected Product (contains) | `Grizzly Bear` | `Grizzly Bear` | `Grizzlybär` | `Grizzlybjörn` | `ピンクのダリア` |
| Expected Currency Symbol | `$` | `£` | `€` | `kr` | `￥` |
| Expected Description Language | English | English | German | Swedish | Japanese |
...
### 7. Cross-Locale Validation (No Bleed)

- **Action:** While in the current locale, search for a term from a *different* language in the Test Data table (e.g., search for the German term `Bär` while in the `en-US` locale).
- **Verify:** The search should return **0 results** (the empty state). A localized term should not match products in an unrelated locale, confirming that the catalogs/indexes are cleanly separated by language.
