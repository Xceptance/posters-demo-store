...
4. Save or update these collective findings directly in the respective domain folder (e.g., `doc/tests/DOMAIN/ISTQB_REVIEW.md`) using the exact format defined in `doc/tests/TEMPLATE_ISTQB_REVIEW.md`.
5. When making edits to specific test cases based on the review, you MUST increment the version in `## Metadata` and append your change to the `## Change History` table as specified in the README rules.
