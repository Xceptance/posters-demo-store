...
public class SideNavValidator
{
	...
	// size of the list
	List<HtmlElement> topCategories = HtmlPageUtils.findHtmlElements(page,
		"id('sidebarNav')/ul/li[@class='topCategory']");
	Assert.assertTrue(
		"There are less then two top categories in the side nav.",
		topCategories.size() >= 2);

	// Check that each top category is followed by at least one level-1
	// category
	for (Iterator<HtmlElement> iterator = topCategories.iterator(); iterator
		.hasNext();)
	{
    ...
    /**
     * The instance for easy reuse. Possible because this validator is
     * stateless.
     * 
     * @return the instance
     */
